...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, num):
        ...
        # prompt = prompt.repeat(WB//B, 1, 1)
        # domain_prompt = domain_prompt.repeat_interleave(WB // B, dim=0)
        # task_prompt = task_prompt.repeat_interleave(WB // B, dim=0)
        # x_windows = torch.cat((domain_prompt, x_windows), dim=1)  # nW*B, window_size*window_size+1, C
        # x_windows = torch.cat((task_prompt, x_windows), dim=1)
        
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, num, mask=self.attn_mask)  # nW*B, window_size*window_size, C
    ...
    def forward(self, x, num):
        for blk in self.blocks:
            if self.use_checkpoint:
                x = checkpoint.checkpoint(blk, x, num)
            else:
                x = blk(x, num)
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x, num):
        x = self.patch_embed(x)     #[b,3,352,352] → [b,7744,96]
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)
        
        x_1_4, x_1_8 = self.layers[0](x, num[0])    #1/4:[b,7744,96]
        x_1_8, x_1_16 = self.layers[1](x_1_8, num[1])  #1/8:[b,1936,192]
        x_1_16, x_1_32 = self.layers[2](x_1_16, num[2])  #1/16:[b,484,384]
        x_1_32 = self.layers[3](x_1_32, num[3]) #1/32:[b,121,768]
    ...
    def forward(self, x, num):
        x_1_4, x_1_8, x_1_16, x_1_32 = self.forward_features(x, num)
        x = self.head(x_1_32)
        return x_1_4, x_1_8, x_1_16, x_1_32
...
def swin_transformer(pretrained=True, **kwargs):  # adopt transformers for tokens to token
    ...
    model = SwinTransformer(img_size=352, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24],
                            window_size=11, drop_path_rate=0.1)
    # model.default_cfg = default_cfgs['swin_transformer']
    args = kwargs['args']
    args.pretrained_model = '/data/clb/VSCode/pretrained_model/swin_tiny_patch4_window7_224.pth'
    if pretrained:
